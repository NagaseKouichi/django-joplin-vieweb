...
class NotesList extends EventEmitter {
    ...
    note_selected(ev) {
        ...
        let is_todo = String($(ev.currentTarget).data('is-todo') ?? false).toLowerCase() === 'true';
        let todo_completed = String($(ev.currentTarget).data('todo-completed') ?? false).toLowerCase() === 'true';
    ...
    select(note_id) {
        ...
        if (selected_li_target.length) { // else is not an error: when note is deleted.
            ...
            super.emit("note_selected", [
                ...
                String(selected_li_target.data("is-todo") ?? false).toLowerCase() === 'true',
                String(selected_li_target.data("todo-completed") ?? false).toLowerCase() === 'true'
            ]);
        }
    }
